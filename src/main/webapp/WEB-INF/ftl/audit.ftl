...
<script>
    ...
    layui.table.render({
        ...
        cols :[[ //列描述
            {title : "序号" , width:70 , style : "height:60px" , type:"numbers"}, // numbers代表序号列
            {field : "create_time" , title : "申请时间" , width : 150 , templet: function (d) {
                //templet代表对数据进行加工后再显示
                return formatDate(d.create_time)
            }},
            {field : "form_type" , title : "请假类型" , width : 100 , templet: function(d){
                switch (d.form_type) {
                    case 1:
                        return "事假";
                    case 2:
            ...
            success: function (json) {
                ...
                if(json.code === "0"){
                    swal({
                        ...
                        window.location.href="/forward/audit";
                    });
                }else{
                    swal({
                        title: "警告",
